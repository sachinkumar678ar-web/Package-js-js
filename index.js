...
// --- 1. Smart Sync (Din mein sirf 1-2 baar read karega) ---
async function syncLocalHistory() {
    try {
        console.log("🔄 Quota Saver: Syncing from Firebase...");
        const q = query(collection(db, 'history_v3'), orderBy('issueNumber', 'desc'), limit(15000));
...
// --- 2. AI Engine (Memory Based - Super Fast) ---
...
async function loop() {
    ...
    try {
        ...
        for (let item of list) {
            ...
            // localHistory check karega (0 Firebase Read)
            if (!localHistory.find(h => h.issueNumber === id)) {
                // Naya data Firebase mein daalna (Only 1 Write per period)
        ...
        // State management with minimal reads
...
setInterval(loop, 18000); // Thoda gap rakhein stability ke liye
setInterval(syncLocalHistory, 21600000); // 6 ghante mein 1 baar full sync
